...
function repage() {
    ...
    let explain_box = document.getElementById("explanation");
...
// write a fucntion to check the answer when click on the image
...
function checkAnswer(event) {
    if (click_once == false) {
        let user_answer = event.target.alt;
        let correct = false;
        let question = questions[counter-1];
        console.log(question);
        for (let i = 0; i < 4; i++) {
            if (question.answer[i].id == user_answer) {
        ...
        if (correct) {
            ...
            explain_box.innerHTML = explanation;
...
// write a function to shuffle the array of questions
